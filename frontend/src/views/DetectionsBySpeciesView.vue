  ...
  <div class="space-y-6">
    <div class="bg-white shadow-sm rounded-lg overflow-hidden">
      <div class="px-6 py-4 border-b border-gray-200">
        <h2 class="text-lg font-medium text-gray-900">{{ commonName }} Detections for {{ formattedDate }}</h2>
      </div>
...
onMounted(async () => {
  try {
    const [detectionsResponse, nameResponse] = await Promise.all([
      axios.get(`/api/detections/by-species/${route.params.scientificName}/${route.params.date}`),
      axios.get(`/api/species/${route.params.scientificName}/common-name`)
    ])
    detections.value = detectionsResponse.data
    commonName.value = nameResponse.data.common_name
  } catch (error) {
