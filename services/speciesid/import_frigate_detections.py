...
def get_frigate_events(start_date=None, end_date=None):
    ...
    url = f"{config['frigate']['frigate_url']}/api/events"
...
def insert_detection(cursor, event, quality_data):
    ...
    try:
        ...
        """, (
            event['start_time'],
            event['scientific_name'],  # Using scientific name from species identification
            event['score'],
            event['id'],
            'bird',
            event['camera'],
...
def import_detections(start_date=None, end_date=None):
    ...
    # Initialize quality model and enhancer (placeholder implementations)
    class BasicQualityModel:
        def assess_quality(self, image):
            # Match the expected quality scores structure
            return {
                'clarity': 0.8,
                'composition': 0.8,
                'overall': 0.8,
    ...
    # Initialize services with image processing config
    quality_model = BasicQualityModel()
    enhancer = BasicEnhancer()
    image_processor = ImageProcessingService(config['image_processing'], quality_model, enhancer)
    ...
    try:
        ...
        for i, event in enumerate(events, 1):
            ...
            try:
                # Process image
                image_url = f"{config['frigate']['frigate_url']}/api/events/{event['id']}/snapshot.jpg"
                ...
                event['scientific_name'] = species_name if species_name else 'Unknown Bird'
